...
nbest_result = sys.argv[1]
asr_result_fn = sys.argv[2]
result_folder = sys.argv[3]
try:
    drop_ngram = sys.argv[4]
    drop_ngram = True
except:
    drop_ngram = False

asr_result = json.load(open(asr_result_fn, 'r', encoding='utf-8'))

score_norm = True


all_beam_result = json.load(open(nbest_result, 'r', encoding='utf-8'))
beam_num = len(all_beam_result["utts"][ list(all_beam_result["utts"].keys())[0]]["output"][0]["detector_prob"][0])

def is_all_same(pos_result):
    for iter_pos in pos_result:
        if iter_pos[0] != pos_result[0][0]:
            return False
    return True

def extract_flag(all_pos, begin, end):
    return [1 for i in range(end - begin)]

def extract_text(all_pos, begin, end, left_allow, right_allow):
    score2rank = [sum([all_pos[j][i][3] for j in range(begin - left_allow, end + right_allow)]) for i in range(len(all_pos[begin]))]
    max_score = max(score2rank)
    for i in range(len(all_pos[begin])):
        if score2rank[i] == max_score:
            break
    return [all_pos[j][i][0] for j in range(begin, end) if all_pos[j][i][0] != '<void>'], [all_pos[j][i][1] for j in range(begin, end) if all_pos[j][i][0] != '<void>'], [all_pos[j][i][2] for j in range(begin, end) if all_pos[j][i][0] != '<void>'], [all_pos[j][i][3] for j in range(begin, end) if all_pos[j][i][0] != '<void>']
    
    

#all_thre = [0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6]
all_thre = [1.3]

for thre in all_thre:
    count = 0
    begin_time = time.time()
    rescore_dict = {"utts": {}}
    for key in all_beam_result["utts"].keys():
        all_final_score = []
        #all_am_score = list(map(float, all_beam_result[0]["utts"][key]["output"][0]["score"].split(';')))
        all_detector_score = []
        all_hypo = []
        for i in range(beam_num):
            all_hypo.append(all_beam_result["utts"][key]["output"][0]["rec_token"].split(' ||| ')[i].split())
        all_source = all_hypo
        all_score = [[np.log(all_beam_result["utts"][key]["output"][0]["detector_prob"][j][i]) for j in range(len(all_beam_result["utts"][key]["output"][0]["detector_prob"]))] for i in range(beam_num)]
        token_num = len(all_source[0])
        for i in range(beam_num):
            assert token_num == len(all_source[i])
        all_pos = [[[0, 0, 0, 0] for _ in range(beam_num)] for _ in range(token_num)]
        for i in range(beam_num):
            iter_score = 0
            asr_full_scores = asr_result["utts"][key]["output"][i]["full_scores"]
            for j in range(len(all_source[i])):
                if all_source[i][j] == '<void>':
                    all_pos[j][i][0] = all_source[i][j]
                    all_pos[j][i][1] = -0.00000001
                    all_pos[j][i][2] = -0.00000001
                else:
                    all_pos[j][i][0] = all_source[i][j]
                    all_pos[j][i][1] = all_score[i][j]
                    if drop_ngram:
                        all_pos[j][i][2] = 0.5 * asr_full_scores["ctc"][iter_score + 1] + 0.5 * asr_full_scores["decoder"][iter_score + 1]
                    else:
                        all_pos[j][i][2] = 0.5 * asr_full_scores["ctc"][iter_score + 1] + 0.5 * asr_full_scores["decoder"][iter_score + 1] + 0.3 *  asr_full_scores["ngram"][iter_score + 1]
                    iter_score += 1
                all_pos[j][i][3] = all_pos[j][i][2] + all_pos[j][i][1] * thre
            assert iter_score == len(asr_full_scores["ctc"]) - 2
        same_flag = [is_all_same(i)  for i in all_pos]
        final_flag = []
        final_text = []
        final_am_score = []
        final_lm_score = []
        ...
        for i in range(len(all_pos)):
            if same_flag[i]:
                if non_same is None:
                    final_flag.append(0)
                    final_text.append(all_pos[i][0][0])
                    final_am_score.append(all_pos[i][0][2])
                    final_lm_score.append(all_pos[i][0][1])
                    final_score.append(all_pos[i][0][3])
                else:
                    final_flag.extend(extract_flag(all_pos, non_same, i))
                    new_text, new_lm_score, new_am_score, new_score = extract_text(all_pos, non_same, i, min(2, non_same), min(2, len(all_pos)-i))
                    final_text.extend(new_text); final_lm_score.extend(new_lm_score); final_am_score.extend(new_am_score); final_score.extend(new_score)
                    final_flag.append(0)
                    final_text.append(all_pos[i][0][0])
                    final_am_score.append(all_pos[i][0][2])
                    final_lm_score.append(all_pos[i][0][1])
                ...
                if non_same is None:
                    ...
                    pass
        if non_same is not None:
            final_flag.extend(extract_flag(all_pos, non_same, i + 1))
            new_text, new_lm_score, new_am_score, new_score = extract_text(all_pos, non_same, i + 1, min(2, non_same), 0)
            final_text.extend(new_text); final_lm_score.extend(new_lm_score); final_am_score.extend(new_am_score); final_score.extend(new_score)
        #assert len(final_flag) == len(all_pos)

        rescore_dict["utts"][key] = copy.deepcopy(all_beam_result["utts"][key])
        rescore_dict["utts"][key]["output"][0]["rec_token"] = " ".join(final_text)
        rescore_dict["utts"][key]["output"][0]["rec_text"] = "".join(final_text)
        if "detector_prob" in rescore_dict["utts"][key]["output"][0].keys():
            del rescore_dict["utts"][key]["output"][0]["detector_prob"]
        rescore_dict["utts"][key]["output"][0]["am_score"] = ";".join(map(str, final_am_score))
        rescore_dict["utts"][key]["output"][0]["lm_score"] = ";".join(map(str, final_lm_score))
        rescore_dict["utts"][key]["output"][0]["score"] = ";".join(map(str, final_score))
        
        count += 1
        if count % 100 == 0:
            print(count, "in", time.time() - begin_time, "s")

    os.makedirs(os.path.join(result_folder, "Thre" + str(thre)), exist_ok=True)
    json.dump(rescore_dict, open(os.path.join(result_folder, "Thre" + str(thre), 'data.json'), 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
