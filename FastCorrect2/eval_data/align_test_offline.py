...
with open('chinese_char_pinyin.txt', 'r', encoding='utf-8') as infile:
  for line in infile.readlines():
    line = line.strip().split('\t')
    ...
    char = line[1]
    freq = int(line[2])
    all_pinyin = list(set([ i[:-1] if i[-1] in ['1', '2', '3', '4'] else i for i in line[4].split('/')]))
    all_pinyin = [i for i in all_pinyin if ':' not in i]
    assert len(all_pinyin) > 0, line
    if char not in g2pM_dict.keys():
      g2pM_dict[char] = all_pinyin[0]
...
#    for line in infile.readlines():
#        all_sentence = line.strip().split('\t')
#        assert len(all_sentence) == 5
#        all_hypo_line.append([sp.encode("".join(i.split()), out_type=str) for i in all_sentence[1:]])
#        all_ref_line.append(sp.encode("".join(all_sentence[0].split()), out_type=str))
...
@set_timeout(300000, after_timeout)  # 限时 20 秒超时
