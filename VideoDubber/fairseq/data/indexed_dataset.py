...
def infer_dataset_impl(path):
    if IndexedRawTextDataset.exists(path):
        return "raw"
    elif IndexedRawLengthDataset.exists(path):
        return "raw_length"
...
def make_dataset(path, impl, fix_lua_indexing=False, dictionary=None):
    ...
    elif impl == "raw_length" and IndexedRawLengthDataset.exists(path):
        assert dictionary is not None
        return IndexedRawLengthDataset(path, dictionary)
    return None
...
def dataset_exists(path, impl):
    ...
    elif impl == "raw_length":
        return IndexedRawLengthDataset.exists(path)
    else:
...
class IndexedRawLengthDataset(FairseqDataset):
    """Takes a text file as input and binarizes it in memory at instantiation.
    Original lines are also kept in memory"""
    ...
    def read_data(self, path, dictionary):
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if _line[-1]=='':
                    _line[-1] = '0'
                _line=list(map(int,_line))
                tokens=torch.LongTensor(_line)
                self.tokens_list.append(tokens)
                self.sizes.append(len(tokens))
        self.sizes = np.array(self.sizes)
